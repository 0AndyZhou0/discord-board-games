...
class Connect4Minimax:
    ...
    def minimax(game: Connect4Game, depth: int, alpha: float = -np.inf, beta: float = np.inf) -> float:
        ...
        for col in range(Connect4.cols):
            ...
            value = -Connect4Minimax.minimax(game, depth - 1, -beta, -alpha)
            start_time = time.time()
    ...
    def get_best_col(game: Connect4Game) -> int:
        ...
        for col in range(Connect4.cols):
            if game.is_column_full(col):
                continue
            row, col = game.drop_piece(col)
            value = -Connect4Minimax.minimax(game, 2)
