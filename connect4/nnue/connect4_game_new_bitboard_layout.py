...
class Connect4Game:
    ...
    def to_bitboards(self, board: np.array) -> tuple[np.longlong, np.longlong]:#FIX
    ...
    def print_bitboard(self) -> None: #FIX
    ...
    def is_column_full(self, col: int) -> bool:
        return self.red_bitboard & (1 << (Connect4.cols - 1) + ((Connect4.cols + 1) * col)) or self.yellow_bitboard & (1 << (Connect4.cols - 1) + ((Connect4.cols + 1) * col))
    ...
    def get_valid_cols_mask(self) -> np.array:#FIX
    ...
    def drop_piece(self, col: int) -> tuple[int, int]:#FIX
        r = None
        c = col
    ...
    def remove_piece(self, row: int, col: int) -> None:#FIX
        player = -self.player
        match player:
            case Color.RED:
    ...
    def check_for_win(self, player: Color) -> bool:#FIX
    ...
    def get_winner(self) -> Color | None:#FIX
