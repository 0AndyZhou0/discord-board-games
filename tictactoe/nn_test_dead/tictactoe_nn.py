...
class TicTacToeNNWrapper:
    ...
    def evaluate_board(self, board: list[list[int]]) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def train(self, train_set: list[tuple[torch.Tensor, torch.Tensor]], epochs: int, batch_size: int, optimizer: torch.optim.Optimizer = None, criterion_prob: torch.nn.MSELoss = None, criterion_eval: torch.nn.MSELoss = None) -> None:
        if optimizer is None:
            optimizer = torch.optim.Adam(self.nn.parameters())
        if criterion_prob is None:
            criterion_prob = torch.nn.MSELoss()
        ...
        for epoch in range(epochs):
            ...
            for batch in range(batch_count):
                ...
                # print("Input Shapes:", train_prob.shape, train_eval.shape)
                policy, evaluation = self.nn(boards)
                # print("Output Shapes:", policy.shape, evaluation.shape)
                # policy_loss = criterion_prob(policy, train_prob)
                # evaluation_loss = criterion_eval(evaluation, train_eval)
